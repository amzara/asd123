...
class _Function1State extends State<function1> {
  ...
  Future<void> fetchData() async {
    ...
    String url =
        'http://10.16.1.22/crp2.php?ext=$ext&domain=trp.uc.dotdashtech.com';
    // Perform GET request
  ...
  Future<void> copyImageToClipboard(extractedImgUrl) async {
    final bytes = await _readImageBytes(extractedImgUrl!);
    if (bytes != null) {
      final clipboard = SystemClipboard.instance;
      if (clipboard != null) {
        final item = DataWriterItem(suggestedName: 'Doraemon.png');
                  ...
                  ElevatedButton(
                    onPressed: () {
                      copyImageToClipboard(
                          extractedImgUrl); // Call method to copy image to clipboard
                    },
